...
def test_replace_pixels_with_frame_local_median_sparse_stack():
    """Flagged pixels get the frame-local median; other pixels are untouched (in place)."""
    from neunorm.filters.gamma_filter import replace_pixels_with_frame_local_median

    rng = np.random.default_rng(42)
    values = rng.poisson(1.0, size=(20, 7, 7)).astype("float64")  # sparse, TPX-like
    values[:, 3, 3] += 500.0  # a gamma-contaminated pixel, hot in every frame
    original = values.copy()

    data = sc.DataArray(
        data=sc.array(dims=["tof", "y", "x"], values=values, variances=values.copy(), unit="counts"),
    )
    mask = np.zeros((7, 7), dtype=bool)
    mask[3, 3] = True
    n = replace_pixels_with_frame_local_median(data, sc.array(dims=["y", "x"], values=mask))

    assert n == 1
    # Every frame's flagged pixel now holds the median of its 3x3 neighbors (center excluded)
    for t in range(20):
        neighborhood = original[t, 2:5, 2:5].copy().ravel()
        neighborhood = np.delete(neighborhood, 4)  # drop the flagged center
        np.testing.assert_allclose(data.values[t, 3, 3], np.median(neighborhood))
        np.testing.assert_allclose(data.variances[t, 3, 3], np.median(neighborhood))
    # Unflagged pixels are untouched
    untouched = ~mask
    np.testing.assert_allclose(data.values[:, untouched], original[:, untouched])


def test_replace_pixels_with_frame_local_median_adjacent_flagged():
    """Adjacent flagged pixels are excluded from each other's neighborhoods."""
    from neunorm.filters.gamma_filter import replace_pixels_with_frame_local_median

    values = np.full((3, 5, 5), 2.0)
    values[:, 2, 2] = 900.0
    values[:, 2, 3] = 800.0

    data = sc.DataArray(data=sc.array(dims=["tof", "y", "x"], values=values, unit="counts"))
    ...
    assert n == 2
    # Both replaced with the clean neighbors' median (2.0), not each other's spike
    np.testing.assert_allclose(data.values[:, 2, 2], 2.0)
    np.testing.assert_allclose(data.values[:, 2, 3], 2.0)


def test_replace_pixels_with_frame_local_median_validates_kernel():
    import pytest

    from neunorm.filters.gamma_filter import replace_pixels_with_frame_local_median

    data = sc.DataArray(data=sc.array(dims=["tof", "y", "x"], values=np.ones((2, 4, 4)), unit="counts"))
    mask = sc.array(dims=["y", "x"], values=np.zeros((4, 4), dtype=bool))
    with pytest.raises(ValueError):
        replace_pixels_with_frame_local_median(data, mask, kernel_size=4)
