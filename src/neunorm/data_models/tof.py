...
class BinningConfig(BaseModel):
    ...
    bin_space: Literal['tof', 'energy', 'wavelength'] = Field(
        default='energy',
        description="Binning domain: 'tof' (ns), 'energy' (eV), or 'wavelength' (Å)"
    )
    use_log_bin: bool = Field(
        default=True,
        description="Use logarithmic spacing (True) or linear spacing (False)"
    )
    energy_range: Optional[Tuple[float, float]] = Field(
        default=None,
        description="Energy range (E_min, E_max) in eV. Required for bin_space='energy'"
    )
    wavelength_range: Optional[Tuple[float, float]] = Field(
        default=None,
        description="Wavelength range (λ_min, λ_max) in Angstrom. Required for bin_space='wavelength'"
    )
    tof_range: Optional[Tuple[float, float]] = Field(
        default=None,
        description="TOF range (t_min, t_max) in nanoseconds. Optional for bin_space='tof'"
    )
    ...
    def validate_range_required(self):
        ...
        if self.bin_space == 'energy' and self.energy_range is None:
            raise ValueError(
                "energy_range required when bin_space='energy'. "
                "Provide energy_range=(E_min, E_max) in eV."
            )
